...
length_t BitPackedRepresentation::getRowValue(length_t rowIndex,
                                              uint16_t bitOffset,
                                              uint8_t numBits) const {
    length_t mask = (1ULL << numBits) - 1;
    uint64_t byteIndex = ((uint64_t)rowIndex) * totalBytes +
                         (bitOffset >> 3); // Use shift for division by 8
    uint8_t bitIndex = bitOffset & 7;      // Use bitwise AND for modulo 8
...
void BitPackedRepresentation::setRowValue(length_t rowIndex, length_t value,
    ...
    uint64_t byteIndex = ((uint64_t)rowIndex) * totalBytes +
                         (bitOffset >> 3); // Equivalent to bitOffset / 8
    uint8_t bitIndex = bitOffset & 7;      // Equivalent to bitOffset % 8
